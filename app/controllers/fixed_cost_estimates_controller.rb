class FixedCostEstimatesController < ApplicationController
  ...
  def create
    ...
    respond_to do |format|
      if @fixed_cost_estimate.save
        format.html { redirect_to(@fixed_cost_estimate, :notice => 'Fixed cost estimate was successfully created.') }
  ...
  def update
    ...
    respond_to do |format|
      if @fixed_cost_estimate.update_attributes(params[:fixed_cost_estimate])
        format.html { redirect_to(@fixed_cost_estimate, :notice => 'Fixed cost estimate was successfully updated.') }
        format.xml  { head :ok }
      else
  ...
  def destroy
    ...
    respond_to do |format|
      format.html { redirect_to(fixed_cost_estimates_url) }
      format.xml  { head :ok }
    end
  end
end
