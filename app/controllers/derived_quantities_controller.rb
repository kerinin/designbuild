class DerivedQuantitiesController < ApplicationController
  ...
  def create
    ...
    respond_to do |format|
      if @derived_quantity.save
        format.html { redirect_to(@derived_quantity, :notice => 'Derived quantity was successfully created.') }
  ...
  def update
    ...
    respond_to do |format|
      if @derived_quantity.update_attributes(params[:derived_quantity])
        format.html { redirect_to(@derived_quantity, :notice => 'Derived quantity was successfully updated.') }
        format.xml  { head :ok }
      else
  ...
  def destroy
    ...
    respond_to do |format|
      format.html { redirect_to(derived_quantities_url) }
      format.xml  { head :ok }
    end
  end
end
