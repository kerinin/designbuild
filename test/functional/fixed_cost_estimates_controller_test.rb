...
class FixedCostEstimatesControllerTest < ActionController::TestCase
  ...
  test "should fail to create xhr unit cost estimate" do
    assert_no_difference('FixedCostEstimate.count') do
      xhr :post, :create, :component_id => @component, :fixed_cost_estimate => {
        :name => nil, :raw_cost => 20
      }
    end
    assert_response(:success)
    assert_template(:create)
  ...
  test "should fail to update xhr unit cost estimate" do
    xhr :put, :update, :component_id => @component, :id => @fixed_cost_estimate.to_param, :fixed_cost_estimate => {
        :name => nil, :raw_cost => 20
      }
    assert_response(:success)
    assert_template(:update) 
